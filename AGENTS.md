...
4. `src/analysis/worklet.js` feeds raw mono PCM to the pure `AnalysisEngine` using a 2048-sample Hann window and 512-sample hop. Features are posted at the audio clock's fixed cadence (about 94 Hz at 48 kHz); `frame(now)` only reads the most recent result. Calibration removes RMS power and per-bin spectral power before detectors and normalization; spectra cross to the main thread only during the three-second calibration capture.
