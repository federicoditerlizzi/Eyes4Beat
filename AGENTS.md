...
- `src/main.js` owns UI, editable settings, repository/sync, packages and shortcuts. It sends serialized commands through `EngineSession` and LocalTransport/RemoteTransport and renders full engine snapshots.
- `src/engine/engine.js` owns the single active runtime and command/event API. `renderer.js`, `audio.js`, `sequencer.js` and `effects.js` own the extracted runtime subsystems; `protocol.js` validates sequenced commands. `src/ui/engine-view.js` paints engine snapshots. See `docs/engine-refactor.md` for ownership, protocol and the manual regression checklist.
...
- `src/library/local-repository.js` owns all active library IndexedDB access; `src/library/runtime.js` builds renderer arrays by project order; `src/library/package-mapping.js` normalizes imported archetypes.
...
The control UI starts with a local engine. Connecting `?output=1` disposes that engine and moves audio, analysis and rendering into the output; the control becomes a remote over a private MessageChannel. Closing or losing output shows a disconnect notice; a local engine is recreated only after the user selects Use this window. Both transports use the same structured-clone command/event boundary. The UI must not access engine internals or runtime arrays. `updateArchetype` is sent immediately on edits, before persistence debounce. The engine reads media blobs itself from the same-origin cache. Full state snapshots are emitted periodically at about 20 Hz and on command completion. Audio feature extraction runs at audio rate in an `AudioWorklet`:

1. The user selects either a local file for the engine-owned `<audio>` element or a live `MediaStream` audio input.
...
`src/entry.js` loads either the control UI or `src/output.js` for `?output=1`. Output contains only canvases, blackout overlay, a one-time Click to start audio unlock, and fullscreen control. A same-origin hello with a session nonce and exact opener identity establishes a private MessageChannel. `RemoteTransport` waits for ready, forwards sequenced commands and returns full versioned status snapshots. Output-local B/P safety actions enter the same engine command stream. No frame replication, remote sequence clock, effect or particle state transfer remains.

The output host survives port detach and control reload/close. A session nonce in control sessionStorage and an origin-local BroadcastChannel announcement rediscover the output through window.opener; the replacement MessageChannel receives authoritative recovery data (project records, controls, audio Blob and a full status snapshot). UI adoption never sends a load or seek to the surviving engine. Snapshot acknowledgements bound queued status messages when control is busy. Heartbeats use a roughly two-second timeout; explicit pagehide notices handle normal close. Disconnected commands reject visibly, and local takeover is always a user choice. Diagnostics show heartbeat age and command RTT.

The control preview captures an output-owned composite canvas (WebGL + particles + blackout) into a muted video at 30 fps. Copying occurs immediately after rendering, without preserveDrawingBuffer. A four-fps ImageBitmap fallback closes every bitmap. Preview is toggleable, automatically stopped while control is hidden, and cleaned up on detach. The engine continues independently of preview.

`EngineSession` retains editable project records, control settings and the loaded audio Blob in the control window. It disposes the old engine before constructing the destination, buffers commands entered during handover, and replays project/media positions, selection, routing/looks, controls, safety, quality and audio transport on fallback. The engine owns its media URLs and reads blobs from the supplied per-user same-origin cache; missing blobs are requested over the port from control, where repository/network access remains. Output enumerates and opens live devices. Audio permission and the initial audio unlock belong to output. Disposal cancels RAF and pending texture decodes, deletes GPU resources, stops media/live tracks, closes the worklet/context, removes device/context listeners and revokes URLs.

Archetype texture changes decode into staging media and commit both roles atomically. Superseded selections are discarded before commit; media generation and slot tokens prevent old image loads from overwriting a newer selection.

Controller-only live shortcuts are resolved in `src/shortcuts.js`: number-row direct archetype selection, arrows or A/D archetype navigation, Alt/Option plus the same number/navigation keys for presets, S/C transition mode and `?` help. They must stay disabled for repeated keydown events, editable controls, open dialogs and output mode. Archetype loading is serialized in `selectArchetype()` so rapid commands resolve to the latest queued selection without racing GPU texture uploads.
...
BLACKOUT and PANIC are latched live-safety controls. BLACKOUT is a DOM overlay independent of WebGL; PANIC uses the pure `src/show-safety.js` calculation to override routed targets without mutating configuration. While PANIC is active, archetype transitions resolve immediately and `updateImageSequence()` only finishes an already-running media transition. B/P shortcuts follow the same typing, dialog, repeat and output-mode guards as the other controller shortcuts. The active engine applies safety; both control commands and output-local B/P shortcuts reach that same instance.
...
- The WebGL render resolution defaults to 78% of the viewport and is device-local adjustable from 50–100% in the header OUTPUT SETTINGS dialog, alongside bloom quality. The active renderer persists both settings; commands and snapshots keep the remote UI in sync. Diagnostics are read-only. Particles use full viewport resolution.
