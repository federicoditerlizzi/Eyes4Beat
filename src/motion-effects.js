...
export const ROTATION_DEFAULTS = Object.freeze({ mode: 'angle', maxAngle: 90, maxSpeed: 90, direction: 'cw', returnToRest: true });
...
export function createRotationState() { return { angle: 0, velocity: 0, sign: 1, idleSince: null, restAngle: 0, active: false }; }
...
export function rotationCover(settings, aspect) {
  ...
  return Math.max(1, Math.cos(angle) + ratio * Math.sin(angle));
}
...
/** Show projects the latest controller snapshot without its own accumulating integrator. */
export function rotationAt(state, settings, elapsed, now) {
  if (!state) return 0;
  if (settings.returnToRest && state.idleSince != null) return state.restAngle * (1 - smooth((now - state.idleSince - 1) / 2));
  return settings.mode === 'spin' ? wrapAngle(state.angle + state.velocity * elapsed) : state.angle;
}
