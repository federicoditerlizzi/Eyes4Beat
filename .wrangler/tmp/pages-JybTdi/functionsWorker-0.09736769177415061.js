...
__name(imageBlob, "imageBlob");
async function onRequestPost({ request, env: env2 }) {
  if (!env2.OPENAI_API_KEY) return json({ error: "Image generation is not configured." }, 503);
  let body;
  try {
...
async function onRequest2(context2) {
  const expectedUser = context2.env.BASIC_AUTH_USERNAME;
  const expectedPassword = context2.env.BASIC_AUTH_PASSWORD;
  ...
  const header = context2.request.headers.get("Authorization") || "";
...
// ../.wrangler/tmp/pages-JybTdi/functionsRoutes-0.4553664028995359.mjs
var routes = [
  {
